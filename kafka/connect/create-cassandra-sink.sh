...
curl -s \
  ...
  "config": {
    ...
    "contactPoints": "cassandradb",
    "loadBalancing.localDc": "datacenter1",
    "topic.spacex.kafkapipeline.spacex_launches.mapping": "launch_id=value.id, mission_name=value.mission_name, launch_date_utc=value.launch_date_utc, rocket_name=value.rocket.rocket_name, launch_success=value.launch_success, site_name=value.launch_site.site_name_long, payload_type=value.rocket.second_stage.payloads[0].payload_type, payload_mass_kg=value.rocket.second_stage.payloads[0].payload_mass_kg",
    "topic.spacex.kafkapipeline.spacex_launches.consistencyLevel": "LOCAL_QUORUM"
  }
}'
echo "Done."